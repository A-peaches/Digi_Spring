...
<%--     <p class="card-text">ID: <%= userData.getUser_id() %></p>
    ...
    <p class="card-text">ID: ${userDto.user_id }</p>
    <p class="card-text">PW: ${userDto.user_pw }</p>
    <p class="card-text">Email: ${userDto.user_email }</p>
    <p class="card-text">Name: ${ userDto.user_name }</p>
    <p class="card-text">Birth: ${userDto.birth }</p>
    <p class="card-text">Hobby: ${userDto.hobby }</p>
    <p class="card-text">Info: ${userDto.info }</p>
  </div>
</div>
</div>
<div class="my-3">
<a href="main" class="btn btn-sm btn-warning mx-3">메인으로</a>
<%-- <%if (userData.getIsadmin()) { %> --%>
<c:if test="${userDto.isadmin}">
    <a href="./memberList" class="btn btn-sm btn-success">회원 목록 조회</a>
</c:if>
<%-- <%} %> --%>
